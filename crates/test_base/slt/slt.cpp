...
int main(int argc, char **argv)
{
  ...
  */
	for (i = 1; i < argc; i++)
	{
		...
		else if (strncmp(z, "-duckdbpath", n) == 0)
		{
			duckdbPath = argv[++i];
		}
  ...
  */
	char *path = (char *)(pEngine->pAuxData);
	if (path != nullptr)
	{
		printf("to connect to the duckdb path: %s\n", path);
	}
	else
	{
		printf("to connect to duckdb in-mem mode\n");
	}
	rc = pEngine->xConnect(path, zConnection, &pConn, zParam);
	if (rc)
	{
  ...
  */
    ...
    ** line number of the first record in sScript.startLine */
      ...
      */
			k = 0;
			while (!nextIsBlank(&sScript) && nextLine(&sScript) && sScript.zLine[0] && strcmp(sScript.zLine, "----") != 0)
			{
				if (k > 0)
					zScript[k++] = '\n';
				memmove(&zScript[k], sScript.zLine, sScript.len);
