...
use crate::compute::kernels::cast_utils::string_to_timestamp32;
use crate::datatypes::*;
        ...
        let vec = (0..string_array.len())
            .map(|i| {
                if string_array.is_null(i) {
                    ...
                    Some(result.map_err(|_| {
                        ArrowError::CastError(
                            format!("Cannot cast string '{}' to value of arrow::datatypes::types::Date32Type ype", string),
                        )
                    }))
                        .transpose()
                }
...
fn cast_string_to_timestamp32<Offset: StringOffsetSizeTrait>(
    array: &dyn Array,
    cast_options: &CastOptions,
) -> Result<ArrayRef> {
    let string_array = array
    ...
    let array = if cast_options.safe {
        let iter = (0..string_array.len()).map(|i| {
            if string_array.is_null(i) {
                ...
                string_to_timestamp32(string_array.value(i)).ok()
            }
        });
        // Benefit:
        //     20% performance improvement
        // Soundness:
        //     The iterator is trustedLen because it comes from an `StringArray`.
        unsafe { Timestamp32Array::from_trusted_len_iter(iter) }
    } else {
        let vec = (0..string_array.len())
            .map(|i| {
                if string_array.is_null(i) {
                    ...
                    let result = string_to_timestamp32(string_array.value(i));
        ...
        // Soundness:
        //     The iterator is trustedLen because it comes from an `StringArray`.
        unsafe { Timestamp32Array::from_trusted_len_iter(vec.iter()) }
    };
