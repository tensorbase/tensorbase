...
pub static BMS: SyncLazy<BaseMgmtSys> = SyncLazy::new(|| {
    ...
    .version("1.0")
    .author("TensorBase")
    .help("TensorBase Server")
    .arg(
        Arg::with_name("conf")
            .short("c")
            .help("Specify the conf file to use")
            .takes_value(true)
    )
    .arg(
        Arg::with_name("conf_string")
            .short("s")
            .help("Specify the string as conf to use, which makes pure-shell users' lifes more easier.")
