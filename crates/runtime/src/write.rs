...
mod unit_tests {
    ...
    mod engine_tests {
        ...
        use base::seq;
        use engine::{
            cg::gen_ker_raw_query,
            jit::run_ker,
            types::{NativeColumn, QueryState},
        };
        use meta::types::{BqlType, ColumnInfo, EngineType, Table, TableInfo};

        //SELECT * FROM system.numbers LIMIT 10;
        //SELECT number FROM system.numbers LIMIT 10;
        //SELECT number FROM system.numbers_0 LIMIT 10;
        //SELECT number FROM system.numbers_0_10;

        #[test]
        fn test_gen_basic() -> BaseRtResult<()> {
            let bms_path = "/jin/tmp/xxx";
            super::prepare_bms(bms_path)?;

            let ms = &BMS.meta_store;
            let ps = &BMS.part_store;
            // let sql = "select number from system.numbers limit 10";
            // let sql = "select sum(number)+max(number) from system.numbers limit 10";
            let sql =
                "select 123*sum(456*number-789)+666 from system.numbers(123)";
            let mut qs = QueryState::default();
            let ker =
                gen_ker_raw_query(&ms, &ps, "default", sql, "abc123", &mut qs)
                    .map_err(|e| BaseRtError::WrappingEngineError(e))?;
            let ker = ker.to_str().unwrap();
            println!("ker:\n{}", ker);
            assert!(ker.contains("const int64_t e = 123;"));
            assert!(ker.contains("int _kerabc123()"));
            assert!(ker.contains((qs.results as u64).to_string().as_str()));

            Ok(())
        }

        #[test]
        fn test_gen_multicols() -> BaseRtResult<()> {
            let bms_path = "/jin/tmp/xxx";
            super::prepare_bms(bms_path)?;
            ...
            // let sql = "select number from system.numbers limit 10";
            // let sql = "select sum(number)+max(number) from system.numbers limit 10";
            let sql = "select 123*sum(456*number-789)+666, count(1) from test_db.test_tab";
            let mut qs = QueryState::default();
            let ker =
                gen_ker_raw_query(&ms, &ps, "default", sql, "abc123", &mut qs)
                    .map_err(|e| BaseRtError::WrappingEngineError(e))?;
